...
if(isset($_POST['btnTimKiem']))
{
    ...
    $list = $CMSNAV->get_list(" SELECT * FROM `orders_caythue` WHERE  `status` = 'xuly' OR `receiver` = '".$getUser['username']."' AND `createdate` >= '".$daterangepicker[0]."' AND `createdate` <= '".$daterangepicker[1]."' ORDER BY id DESC ");
}  
else
{

    $list = $CMSNAV->get_list(" SELECT * FROM `orders_caythue` WHERE `status` = 'xuly' OR `receiver` = '".$getUser['username']."' ORDER BY id DESC ");
