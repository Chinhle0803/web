...
if(isset($_POST['Save']) && $getUser['level'] == 'admin' )
{
    ...
    $row = $CMSNAV->get_row("SELECT * FROM `orders_gamepass` WHERE `id` = '$id' ");
    if(!$row)
    {
    ...
    {
        // refund
        $isMoney = $CMSNAV->cong("users", "money", $row['money'], " `username` = '".$row['username']."' ");
        if($isMoney)
        {
    ...
    $CMSNAV->update("orders_gamepass", array(
        'status' => $status,
        'reason' => $reason
    ), " `id` = '".check_string($_POST['id'])."' ");
...
<main class="app-main"> <div class="wrapper"> <div class="page"> <div class="page-inner"> <div class="page-section"> <div class="card card-fluid"> <div class="card-header">Danh sách đơn hàng gamepass đã hoàn thành</div> <div class="card-body">
                <div class="table-responsive">
                            <table id="datatable1" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $i = 0;
                                    foreach($CMSNAV->get_list(" SELECT * FROM `orders_gamepass` WHERE `status` = 'hoantat' ORDER BY id DESC ") as $row){
                                    ?>
                                    <tr>
                                        <td><?=$i++;?></td>
                                        <td><?=$row['receiver'];?></td>
