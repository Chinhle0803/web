...
if(isset($_GET['delete']) && $getUser['level'] == 'admin')
{
    ...
    $isDelete = $CMSNAV->remove("groups_gamepass", " `id` = '$delete' ");
    if($isDelete)
    {
...
if(isset($_POST['ThemChuyenMuc']) && $getUser['level'] == 'admin' )
{
    ...
    $CMSNAV->insert("groups_gamepass", array(
                        ...
                        <div class="table-responsive">
                            <table id="datatable1" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $i = 0;
                                    foreach($CMSNAV->get_list(" SELECT * FROM `groups_gamepass` WHERE `category` = '".check_string($_GET['id'])."'  ORDER BY id DESC ") as $row){
                                    ?>
                                    <tr>
                                        <td><?=$i++;?></td>
                                        <td><?=$row['title'];?></td>
