<?php
    ...
    $title = 'QUẢN LÝ NHÓM | '.$CMSNAV->site('tenweb');
...
{
    $row = $CMSNAV->get_row(" SELECT * FROM `category` WHERE `id` = '".check_string($_GET['id'])."'  ");
    if(!$row)
    {
                        ...
                        <div class="table-responsive">
                            <table id="datatable1" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $i = 0;
                                    foreach($CMSNAV->get_list(" SELECT * FROM `groups` WHERE `category` = '".check_string($_GET['id'])."'  ORDER BY id DESC ") as $row){
                                    ?>
                                    <tr>
                                        <td><?=$i++;?></td>
                                        <td width="10%"><img width="100%" src="<?=$row['img'];?>" /></td>
                                        <td><?=$row['title'];?></td>
                                        <td><?=format_cash($CMSNAV->num_rows(" SELECT * FROM `accounts` WHERE `groups` = '".$row['id']."' "));?>
                                        </td>
