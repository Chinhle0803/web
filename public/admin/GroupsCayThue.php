<?php
    ...
    $title = 'QUẢN LÝ NHÓM CÀY THUÊ | '.$CMSNAV->site('tenweb');
...
{
    $row = $CMSNAV->get_row(" SELECT * FROM `category_caythue` WHERE `id` = '".check_string($_GET['id'])."'  ");
    if(!$row)
    {
...
if(isset($_GET['delete']) && $getUser['level'] == 'admin')
{
    ...
    $isDelete = $CMSNAV->remove("groups_caythue", " `id` = '$delete' ");
    if($isDelete)
    {
...
if(isset($_POST['ThemChuyenMuc']) && $getUser['level'] == 'admin' )
{
    ...
    $CMSNAV->insert("groups_caythue", array(
                        ...
                        <div class="table-responsive">
                            <table id="datatable1" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $i = 0;
                                    foreach($CMSNAV->get_list(" SELECT * FROM `groups_caythue` WHERE `category` = '".check_string($_GET['id'])."'  ORDER BY id DESC ") as $row){
                                    ?>
                                    <tr>
                                        <td><?=$i++;?></td>
                                        <td><?=$row['title'];?></td>
