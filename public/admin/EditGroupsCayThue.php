<?php
    ...
    $title = 'CHỈNH SỬA NHÓM CÀY THUÊ | '.$CMSNAV->site('tenweb');
...
{
    $row = $CMSNAV->get_row(" SELECT * FROM `groups_caythue` WHERE `id` = '".check_string($_GET['id'])."'  ");
    if(!$row)
    {
...
if(isset($_POST['Save']) && $getUser['level'] == 'admin' )
{
    ...
    $CMSNAV->update("groups_caythue", array(
