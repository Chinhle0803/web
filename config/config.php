<?php
session_start();
$base_url = 'https://test.taphoacode.com/'; // Thay url web bạn
class CMSNAV
{
    private $ketnoi;
    function connect()
    {
        if (!$this->ketnoi)
        {
            $this->ketnoi = mysqli_connect('localhost', 'tsttaphoacode_testshop', 'tsttaphoacode_testshop', 'tsttaphoacode_testshop') or die ('Taphoacode.com!');
...
/* ĐƠN VỊ THIẾT KẾ WEB WWW.CMSNAV.CO | ZALO 0947838128 | FB.COM/NTGTANETWORK */
if(isset($_SESSION['username']))
{ 
    $CMSNAV = new CMSNAV;
    $getUser = $CMSNAV->get_row(" SELECT * FROM users WHERE username = '".$_SESSION['username']."' ");
    $my_username = True;
